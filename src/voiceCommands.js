...
//       await this.attachRecorder(GUILD, textChannel);
//     }
//     //If a song gets queued while player is in idle state (e.g. within the 5 minutes of no songs in que)
//     if (
//       GUILD.audioPlayer &&
//       GUILD.audioPlayer.state.status == AudioPlayerStatus.Idle
//     ) {
//       await this.attachRecorder(GUILD, textChannel);
//     }
//   };
//   skip = (GUILD, textChannel) => {
...
//       return;
//     }
//     let lastPage = Math.floor((GUILD.songQueue.length - 1) / 20);
//     if (page > lastPage) {
//       page = lastPage;
//     }
//     let list = "";
...
//         GUILD.audioPlayer = null;
//         if (GUILD.playingMessageID != null) {
//           GUILD.playingMessageID
//             .delete()
//             .then(() => {})
//             .catch((err) => {
//               console.log(err);
//             });
//           GUILD.playingMessageID = null;
//         }
//       }, 300000);
//       return;
//     }
...
//           console.log(err);
//         });
//       GUILD.playingMessageID = await textChannel.send(
...
//           return songs;
//         }
//         if (song.includes("album")) {
//           //get albumID
//           let albumID = song.substring(31);
//           //incase there is a slash on the end
//           albumID = albumID.replace("/", "");
//           const songsFetch = await (
//             await fetch
//           ).default(`https://api.spotify.com/v1/albums/${albumID}`, {
//             method: "GET",
//             headers: {
//               Authorization: "Bearer " + token,
//             },
...
//           }
//           const songsJSON = await songsFetch.json();
//           console.log(songsJSON.tracks.items);
//           let artists = songsJSON.artists.map((artist, index) => {
//             if (index > 0) {
//               return ", " + artist.name;
//             }
//             return artist.name;
//           });
//           let songs = songsJSON.tracks.items.map((object) => {
//             return object.name + " " + artists;
//           });
//           textChannel.send(`Queued ${songs.length} songs`);
//           return songs;
//         }
